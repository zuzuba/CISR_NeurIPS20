...
__all__ = ['plot_ci', 'set_figure_params', 'emulate_color',
           'linewidth_in_data_units', 'adapt_figure_size_from_axes',
           'cm2inches', 'hide_all_ticks', 'hide_spines',
           'set_frame_properties']


def plot_ci(x, mu, sigma, c='b', lw=2, marker=None, fig=None, ax=None,
            label=''):
    if fig is None:
        fig = plt.gcf()
    if ax is None:
        ax = plt.gca()
    if marker is None:
        ax.plot(x, mu, c=c, lw=lw, label=label)
    else:
        ax.plot(x, mu, c=c, lw=lw, marker=marker, label=label)
    l = mu - 1 * sigma
    u = mu + 1 * sigma
...
def set_figure_params(serif=True, fontsize=9):
    ...
    params = {
        ...
        'font.sans-serif': ['Times',
                            'Helvetica',
                            'Avant Garde',
                            'Computer Modern Sans serif'] + rcParams[
                               'font.sans-serif'],
        'font.family': 'serif',
        'text.usetex': False,
